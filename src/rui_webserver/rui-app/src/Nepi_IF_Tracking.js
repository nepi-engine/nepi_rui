...
// Component that contains the AiTracking controls
class Nepi_IF_AiTracking extends Component {
  detector_info = []
  constructor(props) {
    super(props)
...
renderDetectorSettings() {
  ...
  renderConfigs(){
    const aitrackingNamespace = this.state.aitrackingNamespace
    return(
      <Columns>
      <Column>
  ...
  render() {
    ...
    var has_aitracking = false
    if ((status_msg != null)){
      if (status_msg.aitracking_list.length > 1) {
        has_aitracking = true
      }
      else if (status_msg.aitracking_list.length === 1 && status_msg.aitracking_list[0].name_str !== 'None') {
        has_aitracking = true
      }
    }
...
export default Nepi_IF_AiTracking
