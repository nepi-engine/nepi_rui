...
class Nepi_IF_AiDetectors extends Component {
  detector_info = []
  constructor(props) {
    super(props)
...
renderDetectorSettings() {
  ...
  render() {
    ...
    const aidetectors_view_restricted = userRestricted.indexOf('SYSTEM-AIDETECTORS-CONTROL') !== -1 && (ignore_restrictions === false)

    const title = (this.props.title !== undefined) ? this.props.title : "AI Detector Settings"

    if (aidetectors_view_restricted === true){
      return (
        <Columns>
        <Column>
...
export default Nepi_IF_AiDetectors
